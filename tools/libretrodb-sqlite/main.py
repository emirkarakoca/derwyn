...
class Converter:
    ...
    def _load_sql(self, sql_file):
            # Scriptin bulunduğu dizini baz alarak tam yol oluşturur
            base_dir = Path(__file__).resolve().parent
            full_path = base_dir / sql_file
    ...
    def _parse_line(self, json_str, platform_id):
        try:
            ...
            try:
                ...
                json_obj = json.loads(fixed_str, strict=False)
            except Exception as e:
                try:
                    import re
                    cleaned_str = re.sub(r',?\s*"description"\s*:\s*".*?(?<!\\)"', '', json_str, flags=re.DOTALL)
                    json_obj = json.loads(cleaned_str, strict=False)
                except Exception as final_e:
                    self.logger.error("Error while parsing JSON: {}".format(str(final_e)))
        ...
        description = self._get_json_value(json_obj, 'description') # Artık description da dahil ediliyor
...
if __name__ == '__main__':
    ...
    default_rdb_dir = project_root / "data" / "libretrodb" / "rdb"
    default_output = project_root / "data" / "libretrodb" / "libretrodb.sqlite"
    default_tool = Path(__file__).resolve().parent / "libretrodb_tool"
    parser = argparse.ArgumentParser()
    parser.add_argument('--rdb-dir', type=str, nargs=1, default=[str(default_rdb_dir)])
    parser.add_argument('--output', type=str, nargs=1, default=[str(default_output)])
    parser.add_argument('--libretrodb-tool', type=str, nargs=1, default=[str(default_tool)])
